...
class P9f
{
	...
	function getPrice($options, $tax)
	{
		...
		if(empty($settings->price_thousand)) $settings->price_thousand = '';
		if(empty($settings->price_decimal)) $settings->price_decimal = '';
		foreach($data as $key => $value)
		{
			$price 		= str_replace($settings->price_thousand, '', $value);
			$data->$key = str_replace($settings->price_decimal, '.', $price);
		}
