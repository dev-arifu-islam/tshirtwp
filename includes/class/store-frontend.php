...
class P9f_store_frontend
{
	...
	* Redirect after add to cart in page product detai with design template
	...
	{
		global $P9f, $wc_cpdf;
		$product_design_id = $wc_cpdf->get_value($product_id, '_product_id');
		if( isset($_POST['design_idea_id']) && $_POST['design_idea_id'] != '' )
		{
			$idea_id 	= $_POST['design_idea_id'];
			if(isset($_SESSION["design_idea_".$idea_id.$product_design_id]))
				$design 	= $_SESSION["design_idea_".$idea_id.$product_design_id];
		}
		elseif(isset($_POST['rowid']) && $_POST['rowid'] != 'blank')
		{
			$idea_id 	= $_POST['rowid'];
			if (isset($_SESSION["design_idea_".$idea_id.$product_design_id]))
				$design 	= $_SESSION["design_idea_".$idea_id.$product_design_id];
		}
		elseif (isset($_SESSION["design_idea__blank".$product_design_id]))
		{
				$design 	= $_SESSION["design_idea__blank".$product_design_id];
		}

		if( isset($design) && isset($design['prices']) )
		{
			$prices 	= $design['prices'];
			$data_items['prices_options']	= $prices;

			$rowid 				= md5($idea_id.$product_design_id.time());
			$data_items['design_price']	= $prices->item;
			$data_items['design_id'] 	= $rowid;

			$item			= array(
				'id'			=> '',
				'product_id'	=> $product_design_id,
				'qty'			=> $_POST['quantity'],
			...
			{
				$idea_id 	= str_replace('cart:', '', $idea_id);
				if(strpos($idea_id, ':') > 0)
				{
					$ides = explode(':', $idea_id);
					$idea_id = end($ides); echo $idea_id;
				}
				$vectors 	= $this->getDesign('cart', $idea_id);

				$data = array('fonts' => '');
				if(isset($vectors['vectors']))
				{
					$data['vectors'] = $vectors['vectors'];
				}
				elseif(isset($vectors['vector']))
				{
					$data['vectors'] = $vectors['vector'];
				}
				else
				{
					$data['vectors'] = '';
