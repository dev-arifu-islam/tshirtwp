...
add_action( 'wp_footer', 'tshirtecommerce_product_image_scr' );
function tshirtecommerce_product_image_scr()
{
	if(is_product())
	{
		global $P9f;
		$settings 	= $P9f->product_design;
		echo '<script type="text/javascript">var URL_d_home = "'.home_url().'";var design = {};</script>';

		if( !isset($settings['hide_quickview']) )
		{
			$settings['hide_quickview'] = 0;
		}
		if($settings['hide_quickview'] == 0)
		{
