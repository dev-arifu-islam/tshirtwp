...
function designer_product_attribute($values)
{
	...
	if (count($data))
	{
		...
		if (isset($settings['hide_colors']) && $settings['hide_colors'] == 1)
		{
			return false;
		}
		...
		if (isset($data->design) && isset($data->design->color_hex))
		{
			...
			if (!isset($data->min_order) || $data->min_order < 1)
			{
				$data->min_order = 1;
			}
			if (!isset($data->max_oder) || $data->max_oder < $data->min_order)
				$data->max_oder = 10000;

			$print_type = isset($data->print_type) ? $data->print_type : '';
			if (!empty($print_type)) {
				$current_printing = getTshirtPrintings($print_type);
				if (isset($current_printing->options_extra) && isset($current_printing->options_extra->quantity) && $current_printing->options_extra->quantity > 0) {
					$data->min_order = $current_printing->options_extra->quantity;
				}
			}
