			...
			if ($show_butoon == true)
			{
				...
				else
				{
					if (is_numeric($array[0]))
					{
						...
						{
							echo '<a class="button e-custom-product '.$settings['btn-extra_class'].'" onclick="return loadProductDesign(this);" data-product_id="'.$array[0].'" href="'.$link.'">'.$settings['btn-custom'].'</a>';
						}
						else
						{
							echo '<a class="button e-custom-product '.$settings['btn-extra_class'].'" onclick="return loadProductDesign(this);" data-product_id="'.$array[0].'" href="'.$link.'">'.$settings['btn-start'].'</a>';
						}
		...
		else
		{
			...
			elseif( $product->is_type( 'variable' ) )
			{
				if (try_to_count($array) > 1)
				{
					$link = $link. ':'. $product_id;
					$link = add_query_arg( array('design'=>$link), $page );
					echo '<input type="hidden" value="'.$link.'" id="product-design-link">';
				}
				else
				{
					echo '<input type="hidden" value="'.$page.'" id="product-design-link">';
				}
			}
		}
	}
...
function design_button_variable()
{
	...
	if ($link != '' && $product->is_type( 'variable' ))
	{
		...
		$lang 	= $P9f->lang;
		echo '<script type="text/javascript"> var txt_select_variation_product = "'.$lang['select_variation_product'].'"; var txt_out_of_stock_variation_product = "'.$lang['out_of_stock_variation_product'].'" </script>';
	}
}
...
function tshirtecommerce_func( $atts )
{
	...
	if ($product_id == 0)
	{
		$html .= '<div class="alert alert-danger" role="alert">Please add product design in woocommerce. <a href="http://docs.tshirtecommerce.com/videos/how-to-add-new-product-design-in-woocommerce/">View Video</a></div>';
	}
	else
	{
		$product_design = 1;
		...
		elseif(try_to_count($array) > 1)
		{
			...
			if (isset($_GET['cart_id']) && isset($product_id))
			{
				$url = $url.'&parent='.$product_id;
			}
		}
		else
		{
...
function save_custom_field_design( $cart_item_key, $product_id = null, $quantity= null, $variation_id= null, $variation= null ) {
	...
	WC()->cart->cart_contents[$cart_item_key][$cart_item_key.'_designer'] = $data_items;
}
...
function tshirt_order_meta_handler( $item_id, $values, $cart_item_key ) {
	if( WC()->session->__isset( $cart_item_key.'_designer' ) ) {
		...
		if (isset(WC()->cart->cart_contents[$cart_item_key][$cart_item_key.'_designer'])) {
			wc_add_order_item_meta( $item_id, "custom_designer", WC()->cart->cart_contents[$cart_item_key][$cart_item_key.'_designer'] );
		}
	}
}
			...
			if (is_object($images))
			{
				...
				foreach($images as $view => $image)
				{
					...
					if($image != '')
					{
						...
						{
							$image = network_site_url('tshirtecommerce').'/'.$image;
						}
						echo '<img style="width: 100px;" src="'. $image .'" with="100">';
					}
					else
					{
						$wo_images = wp_get_attachment_image_src( get_post_thumbnail_id( $wo_product_id ), 'single-post-thumbnail' );
						echo '<img style="width: 100px;" src="'. $wo_images[0] .'" with="100">';
					}
					echo '<br /><a target="_blank" href="'.network_site_url('tshirtecommerce/design.php?key='.$data['design_id'].'&view='.$view.'&session_id='.session_id()).'">'.$lang[$view].'</a>';
					echo '</td>';
				}
				echo '</tr></table>';
...
function add_custom_price( $cart_object )
{
    foreach ( $cart_object->cart_contents as $key => $value )
	{
		...
		if ($data == null && isset(WC()->cart->cart_contents[$key][$key.'_designer'])) {
			$data = WC()->cart->cart_contents[$key][$key.'_designer'];
		}
