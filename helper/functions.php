	...
	switch($type)
	{
		...
		case 'textlist':
			...
			for ($i=0; $i<count($title); $i++)
			{
				...
				$html .= 	'<input type="number" onkeyup="tshirt_attributes(this, \''.$index.$i.'\')" class="form-control input-sm size-number '.$class_required.'" name="'.$id.'[value]['.$title[$i].']">';
...
function getTshirtPrintings($printing_code)
{
	$data = array();
	$file = ABSPATH .'tshirtecommerce/data/printings.json';
	if (file_exists($file)) {
			...
			if (count($printings)) {
				foreach ($printings as $printing) {
					...
					if ($printing->printing_code == $printing_code) {
						$code_type	= ABSPATH.'tshirtecommerce/addons/printings/'.$printing->price_type.'.json';
